...
def signup():
    ...
    for i in data_account:
        if "".join(i) == account:  # 從tuple轉成str
            return redirect(url_for("error", message="帳號已經被註冊")) # 等同於/error/?message=帳號已經被註冊
    # insert data
...
def signin():
    ...
    data_account = cursor.fetchall()
    for i in data_account:
        if account+password == "".join(i): # 從tuple轉成str
        ...
        return redirect(url_for("error", message="帳號或密碼輸入錯誤")) # 等同於/error/?message=帳號或密碼輸入錯誤
